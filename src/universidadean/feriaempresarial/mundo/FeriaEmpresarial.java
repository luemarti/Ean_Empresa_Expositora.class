...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        // Cuantos puestos hay en la zona CENTRO = cuantos puestos ocupados existen en la zona CENTRO
        int centro_Puestos_Cant =0;
        int norte_Puestos_Cant =0;
        int sur_Puestos_Cant =0;
        int occidente_Puestos_Cant =0;
        int oriente_Puestos_Cant =0;
        int cant_Puestos = 0;
        String NombreZona = "";

        for (Puesto p: puestos){
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_CENTRO)){centro_Puestos_Cant++;}
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_NORTE)){norte_Puestos_Cant++;}
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_SUR)){sur_Puestos_Cant++;}
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_OCCIDENTE)){occidente_Puestos_Cant++;}
            if(p.estaOcupado() && p.darZona().equals(Puesto.ZONA_ORIENTE)){oriente_Puestos_Cant++;}
        }
        cant_Puestos = centro_Puestos_Cant;
        NombreZona = "Zona Centro";
        if(cant_Puestos < norte_Puestos_Cant){
            NombreZona = "Zona Norte";
            cant_Puestos = norte_Puestos_Cant;
        }
        if(cant_Puestos < sur_Puestos_Cant){
            NombreZona = "Zona Sur";
            cant_Puestos = sur_Puestos_Cant;
        }
        if(cant_Puestos < occidente_Puestos_Cant){
            NombreZona = "Zona Occidente";
            cant_Puestos = occidente_Puestos_Cant;
        }
        if(cant_Puestos < oriente_Puestos_Cant){
            NombreZona = "Zona Oriente";
            cant_Puestos = oriente_Puestos_Cant;
        }
        return "Existen " + cant_Puestos + " puestos ocupados en la " + NombreZona + ".";
    ...
    public String metodo2() {
        double porcentaje_Ocupacion = this.porcentajeOcupacion();
        if(porcentaje_Ocupacion < 60)
